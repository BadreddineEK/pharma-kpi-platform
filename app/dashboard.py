...
def render():
    ...
    st.sidebar.title("\U0001f48a Pharma KPI")
    st.sidebar.markdown("---")
    ...
    if page == "Overview":
        ...
        metric = st.selectbox(
            "Metric", ["batch_yield", "cycle_time", "oos_rate", "revenue_index", "adverse_events"]
        )
        fig = px.line(dfs, x="date", y=metric, title=f"{metric.replace('_', ' ').title()} - {site}")
        if metric in THRESHOLDS:
            val = THRESHOLDS[metric][1]
        ...
        if len(hist) < 14:
            ...
            fig.add_trace(go.Scatter(
                x=hist["ds"], y=hist["y"], name="Historical", line=dict(color="#4C9BE8")
            ))
            fig.add_trace(go.Scatter(
                x=forecast["ds"], y=forecast["yhat"],
                name="Forecast", line=dict(color="#F4A261", dash="dash")
            ))
            fig.add_trace(go.Scatter(
            ...
            if metric in THRESHOLDS:
                val = THRESHOLDS[metric][1]
                fig.add_hline(
                    y=val, line_dash="dot", line_color="red",
                    annotation_text=f"Threshold {val}"
                )
            fig.update_layout(
                title=f"{metric.replace('_', ' ').title()} - {horizon}d forecast",
                hovermode="x unified"
            )
            st.plotly_chart(fig, use_container_width=True)
        ...
        if alerts_df.empty:
            ...
            st.error(f"{len(alerts_df)} breach events detected")
